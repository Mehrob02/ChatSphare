...
class AuthService extends ChangeNotifier{
  ...
  Future <UserCredential> signInWithEmailAndPassword(String email, String password)async{
   ...
   await firebaseFirestore.collection("users").doc(userCredential.user!.uid).set({
    ...
    try {
      await firebaseFirestore.collection("users_tokens").doc(userCredential.user!.uid).set({
      "token": fMCToken,
    });
    } catch (e) {
...
Future<UserCredential> signUpWithEmailAndPassword(String email, String password, String nickName) async {
  try {
    ...
    await firebaseFirestore.collection("nickNames").doc(userCredential.user!.uid).set({
      "nickName" : nickName,
    });
    return userCredential;
  } on FirebaseAuthException catch (e) {
